...
sf::st_write(y, './data-raw/shp/MSOA.shp')
...
assign(fn, y)
save( list = fn, file = file.path('data', paste0(fn, '.rda')), version = 3, compress = 'gzip' )
sf::st_write(y, './data-raw/shp/MSOAgb.shp')
...
y1 <- data.table( 
          ...
          y |> st_centroid() |> st_geometry() |> st_transform(4326) |> st_coordinates() |> as.data.table() |> setNames(c('x_lon', 'y_lat'))
      ) |> setkey('MSOA')
...
y2 <- fread(
          ...
          col.names = c('MSOA', 'ox_lon', 'oy_lat')
      ) |> setkey('MSOA')
