...
#' Modify a leaflet map
#' 
#' Start the process to modify the leaflet map in the shiny app, using the trick to trigger the *spin* plugin
#'
...
#' @export
#'
mod_map <- \() leafletProxy('out_map') |> removeShape(layerId = 'spinnerMarker')
...
add_ppop <- function(mp, pb, pr, pn, rv, dn){
    ...
    mp |> 
        clearGroup('gridpop') |> removeControl('legend') |> 
        addGlPoints(,
            data = pb, 
            group = 'gridpop',
            radius = pr,
...
#' @param coords a vector of coordinates values for the centroids
#'
#' @return a leaflet object
#' 
#' @author Luca Valnegri, \email{l.valnegri@datamaps.co.uk}
...
#' @export
#'
add_cmarker <- function(mp, cx, cv, coords = NULL){
    ...
    mp |> 
        ...
        addAwesomeMarkers(
            data = cv, 
            lng = if(is.null(coords)){ ~get(paste0(tx$code, 'x_lon')) } else { coords[1] }, 
            lat = if(is.null(coords)){ ~get(paste0(tx$code, 'y_lat')) } else { coords[2] },
            group = txg,
