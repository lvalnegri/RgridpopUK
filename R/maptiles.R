...
#' @param x   text or url description of the required maptile (see `tiles.lst`)
...
add_maptile <- function(m, x, grp = NULL){
    switch(gsub('.*(google|memomaps|cycl).*', '\\1', x),
        'google' = m |> addTiles(
                            urlTemplate = x, 
        ...
        m |> addProviderTiles(x, group = grp, options = providerTileOptions(useCache = TRUE, crossOrigin = TRUE))
    )
}
